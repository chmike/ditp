...
// tmez return the given time string as time.Time.
func tmez(timeStr string) time.Time {
	t, err := time.Parse("2006-01-02T15:04:05.999999999-07:00MST", timeStr)
	if err != nil {
		panic(err)
	}
	return t
...
func TestEncoder(t *testing.T) {
	tests := []struct {
		...
		// 35
		{t: VarTimeTag, i: tmez("2023-10-06T10:00:00+00:00UTC"), o: []byte{0xa, 0xc0, 0xea, 0xfe, 0xd1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0}},
		{t: VarTimeTag, i: tmez("2023-10-06T10:00:00.5+01:00CET"), o: []byte{0xf, 0xa0, 0xb2, 0xfe, 0xd1, 0xc, 0x0, 0x65, 0xcd, 0x1d, 0xa0, 0x38, 0x3, 0x43, 0x45, 0x54}},
		{t: VarTimeTag, i: tmez("2023-10-06T10:00:00-07:00MDT"), o: []byte{0x10, 0xa0, 0xf4, 0x81, 0xd2, 0xc, 0x0, 0x0, 0x0, 0x0, 0xdf, 0x89, 0x3, 0x3, 0x4d, 0x44, 0x54}},
		{t: VarTimeTag, i: tmez("2023-10-06T10:00:00-05:00EST"), o: []byte{0x10, 0xe0, 0x83, 0x81, 0xd2, 0xc, 0x0, 0x0, 0x0, 0x0, 0x9f, 0x99, 0x2, 0x3, 0x45, 0x53, 0x54}},
		{t: VarTimeTag, i: tmez("2023-10-06T10:00:00+02:00CEST"), o: []byte{0x10, 0x80, 0xfa, 0xfd, 0xd1, 0xc, 0x0, 0x0, 0x0, 0x0, 0xc0, 0x70, 0x4, 0x43, 0x45, 0x53, 0x54}},
		//40
		...
		{t: NoneTag, i: Tag(0), o: []byte{0}},
		{t: NoneTag, i: Tag(12345), o: []byte{0xB9, 0x60}},
	}
	var e Encoder
	e.With(make([]byte, 64))
	for i, test := range tests {
		...
		case NoneTag:
			e.PutTag(test.i.(Tag))
		case BoolTag:
			e.PutBool(test.i.(bool))
		case ByteTag:
			e.PutByte(test.i.(byte))
		case BytesTag:
			e.PutBytes(test.i.([]byte)...)
		case VarUintTag:
			e.PutVarUint(test.i.(uint64))
		case VarIntTag:
			e.PutVarInt(test.i.(int64))
		case SizeTag:
			e.PutSize(test.i.(uint64))
		case VarFloatTag:
			e.PutVarFloat(test.i.(float64))
		case VarComplexTag:
			e.PutVarComplex(test.i.(complex128))
		case Uint8Tag:
			e.PutUint8(test.i.(uint8))
		case Uint16Tag:
			e.PutUint16(test.i.(uint16))
		case Uint32Tag:
			e.PutUint32(test.i.(uint32))
		case Uint64Tag:
			e.PutUint64(test.i.(uint64))
		case Int8Tag:
			e.PutInt8(test.i.(int8))
		case Int16Tag:
			e.PutInt16(test.i.(int16))
		case Int32Tag:
			e.PutInt32(test.i.(int32))
		case Int64Tag:
			e.PutInt64(test.i.(int64))
		case Float32Tag:
			e.PutFloat32(test.i.(float32))
		case Float64Tag:
			e.PutFloat64(test.i.(float64))
		case Complex64Tag:
			e.PutComplex64(test.i.(complex64))
		case Complex128Tag:
			e.PutComplex128(test.i.(complex128))
		case BlobTag:
			e.PutBlob(test.i.([]byte))
		case StringTag:
			e.PutString(test.i.(string))
		case DIRTag:
			e.PutDIR(test.i.(dir.DIR))
		case VarTimeTag:
			e.PutVarTime(test.i.(time.Time))
		case TimeTag:
			e.PutTime(test.i.(time.Time))
		default:
			t.Errorf("%3d unsupported type %T", i, test.i)
			continue
		}
		if e.Len() != len(test.o) {
			t.Errorf("%3d expected len %d, got %d", i, len(test.o), e.Len())
		}
		if !bytes.Equal(e.Bytes(), test.o) {
			t.Errorf("%3d expected encoding %#v, got %#v", i, test.o, e.Bytes())
		}
	}
}
